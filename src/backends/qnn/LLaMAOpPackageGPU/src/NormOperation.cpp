...
const std::string NormOperation::s_operationType = "Norm";

std::shared_ptr<Operation> NormOperation::create(const QnnGpuOpPackage_Node_t* node,
                                                 Qnn_ErrorHandle_t* status) {
  return std::shared_ptr<NormOperation>(new (std::nothrow) NormOperation(node, status));
};

NormOperation::NormOperation(const QnnGpuOpPackage_Node_t* node, Qnn_ErrorHandle_t* status)
    : Operation() {
  ////////////////////////////////////////////////////////////////////////
...
QnnGpu_Kernel_t NormOperation::setKernelInfo(size_t numElements, Qnn_ErrorHandle_t* status) {
  ...
  // ////////////////////////////////////////////////////////////////////////
  // /// Set up the Input int kernel argument
  // ////////////////////////////////////////////////////////////////////////
  // QnnGpu_KernelArg_t InputsizeArg = QNN_GPU_KERNEL_ARG_INIT;
  // InputsizeArg.type = QNN_GPU_KERNEL_ARG_CL_TYPE_INT;
  // InputsizeArg.scalar.dataType = QNN_DATATYPE_INT_32;
  // InputsizeArg.scalar.int32Value = static_cast<int32_t>(numElements);
  // m_kernelArgs.push_back(InputsizeArg);
  ...
  m_kernelSource = R"(
  __kernel void norm_kernel(__global float* input, __global float* output, int N) {
      ...
      float norm = sqrt(sum);
      output[idx] = input[idx] / (norm + 1e-6f);
  })";

  m_kernelName           = "norm_kernel";
